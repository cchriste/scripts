...
#
# idxfastwrite_yfp.sh (only includes C01 and C02)
#
...
VBASE=${IDXDIR}/ntdata     #prefix is a wildcard ????_ and we don't want to use it for the idx name
VFILE=${VBASE}.idx
#rm -Rf $VBASE $VDIR 
#mkdir ${BASEDIR}/idx
...
fields="C01 1*${DTYPE} compressed + C02 1*${DTYPE} compressed"

# get resolution of first file (sets xres, yres)
`ls -1d ${DATADIR}/${PREFIX}*C01${EXT} | head -n 1 | xargs identify | awk '{ split($3, sz, '/x/'); print "export xres=" sz[1]; print "export yres=" sz[2]; }'`

# get number of files (sets numslices)
export numslices=`ls -1d ${DATADIR}/${PREFIX}*C01${EXT} | wc -l`
...
for ((c=1;c<=2;c++)); do 
    ...
    importlist="";
    for f in `ls -1d ${DATADIR}/${PREFIX}*C0${c}${EXT}`; do 
